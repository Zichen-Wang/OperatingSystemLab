...
如果设置网桥仅用来交换数据，那么可以不用设置IP地址。如果设置网桥需要与主机通信或者与Internet通信，则需要分配IP地址，并在主机中设置相应的路由表。例如，在Docker容器中，主机docker0的IP地址为172.17.0.1，作为网关使用，而各个容器的虚拟网卡的IP地址段就是172.17.0.0/24，网桥可以看做网关，容器可以通过网桥访问Internet。
